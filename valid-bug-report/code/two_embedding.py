...
csv_path = '../data/eclipse/data.csv'
w2v_path = '../valid-bug-report/model/word2vec/eclipse/'
t2v_path = '../model/token_index/'
result_path = '../result/eclipse/result.csv'
...
def split_data_ordered(ids, texts, stru, labels):
    br_list = list_split(texts, 10)
    s_list = list_split(stru, 10)
    labels_list = list_split(labels, 10)
    ids_list = list_split(ids, 10)

    return br_list,s_list, labels_list, ids_list
...
def split_train_test(bug_report,stru, labels, id_list):
    br_train = lists_merge_test(bug_report[0:9])
    labels_train = lists_merge_test(labels[0:9])
    s_train = lists_merge_test(stru[0:9])
    br_test = bug_report[9]
    s_test = stru[9]
    labels_test = labels[9]
    id_test = id_list[9]

    return br_train,s_train,labels_train,br_test,s_test,labels_test,id_test

def test_model(model, br_test,s_test, label_test, id_test):
    result = model.predict([br_test,s_test])
    print("result:", result)
    tp = 0
    tn = 0
    fp = 0
...
if __name__ == '__main__':
        ...
        # bug_report = np.concatenate([texts, pro, com, ver, sev, pri, rep, ass, att], axis=1)
        data_concat = np.column_stack((pro, com, ver, sev, pri, rep, ass, att))
        ...
        br_list,s_list, labels_list, ids_list = split_data_ordered(BID, texts, bug_report, labels)
        x_train,s_train, y_train, x_test, s_test,y_test,id_test = split_train_test(br_list,s_list, labels_list, ids_list)
        ...
        with tf.device('/GPU:0'):
            text_inputs = Input(shape=(248,), dtype='int32')
            stru_inputs = Input(shape=(8,), dtype='int32')
            text_embeddings = Embedding(max_features, 120)(text_inputs)
            text_embeddings = Concatenate(axis=-1,name='desConcate1')([text_embeddings,stru_inputs])

            print("\n"*2)
            print("embeddings:")
            print(text_embeddings)

            mask_inputs = padding_mask(text_embeddings)
            print("mask_inputs:")
            print(mask_inputs)
            out_seq = Encoder(12, 128, 4, 256, max_len)(text_embeddings, mask_inputs)
            ...
            model = Model(inputs=[text_inputs,stru_inputs], outputs=outputs)
            print(model.summary())
            ...
            print('Train...')
            history = model.fit([x_train,s_train], y_train,
                     batch_size=batch_size,
                     epochs=3,
                     validation_split=0.1)

            tp, tn, fp, fn, predict, label, id = test_model(model, x_test,s_test, y_test, id_test)
